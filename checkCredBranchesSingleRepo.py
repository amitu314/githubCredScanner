...
if __name__ == "__main__":
    ...
    repoPath = Path('<path to repo onwindows>')
    ...
    with open ('agentFrameworkCred.txt', 'w',encoding='utf-8', errors='ignore') as credFile:
        ...
        if (repoPath / '.git').exists():
            hits = remoteBranchCheckNoCheckout(repoPath, regPattern)
            for branch, matches in hits.items():
                    ...
                    for item in matches:
                        for val in item['matches']:
                            ...
                            credFile.write(f"""===========================================================================================================================================================================================
In Repo {repoPath} under Branch {branch}
        Found in File: {item['path']}
            Match: {val}
""")

        else:
            print(f"{repoPath} is not a git repository")
