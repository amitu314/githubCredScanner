...
def test_respects_extra_skip_ext(tmp_path: Path):
    ...
    # Default behavior: .log is NOT in DEFAULT_SKIP_EXTS, so both should be found
    matches1 = scan_directory(tmp_path, output_file1)
    assert any("logsecret" in m["match"] for m in matches1)
    assert any("txtsecret" in m["match"] for m in matches1)
